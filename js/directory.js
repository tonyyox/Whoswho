...
WhosWho.DirectoryController = {
  ...
  _onViewInChart: null,
  _displayLimit: 100,
  _filters: { query: '', offices: [], depth: null },
  ...
  _buildDepthFilter: function () {
    var self = this;
    var maxDepth = WhosWho.DataUtils.getMaxDepth(this._users);

    var select = document.createElement('select');
    select.className = 'directory-filter';
    ...
    select.addEventListener('change', function () {
      self._filters.depth = select.value === '' ? null : parseInt(select.value, 10);
      self._displayLimit = 100;
      self._applyFilters();
    });

    this._toolbarEl.appendChild(select);
  },
  ...
  _applyFilters: function () {
    ...
    // Depth filter
    if (this._filters.depth !== null) {
      var depth = this._filters.depth;
      results = results.filter(function (u) { return u._depth === depth; });
    }
  ...
  _createCard: function (user) {
    ...
    if (typeof user._depth === 'number') {
      var badge = document.createElement('span');
      badge.className = 'depth-badge';
      badge.textContent = 'Level ' + (user._depth + 1);
