...
WhosWho.generateMockData = function () {
  ...
  // C-suite: 6 executives report to CEO, each overseeing 2 departments
  var cSuite = [
    { title: 'CTO',            depts: ['Engineering', 'Infrastructure'] },
    { title: 'CPO',            depts: ['Product Development', 'Customer Success'] },
    { title: 'CRO',            depts: ['Commercial', 'Consulting'] },
    { title: 'CMO',            depts: ['Marketing', 'Supplier'] },
    { title: 'CFO',            depts: ['Finance', 'Risk'] },
    { title: 'CIO',            depts: ['Information Security', 'IT Support'] }
  ];
  ...
  for (var cs = 0; cs < cSuite.length; cs++) {
    var exec = cSuite[cs];
    var execId = addUser(ceoId, makeName(), exec.title, 'Executive', pick(officeWeights));
